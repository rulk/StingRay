...
void ConfigLoader::_nextToken(std::ifstream &stream)
{
	...
	{
		//!TODO: throw something !throw std::runtime_error("Parse Error: Invalid character, ConfigLoader::load()");
	}
	...
	while (true)
	{
		...
		case TOKEN_Text:
			...
			if (tok == TOKEN_OpenBrace)
			{
				...
				{
					//!TODO:Throw somethingthrow std::runtime_error("Parse Error: Expecting closing brace");
				}
				_nextToken(stream);
				_skipNewLines(stream);
			}
		...
		case TOKEN_OpenBrace:
			//!TODO: throw something throw std::runtime_error("Parse Error: Opening brace out of plane");
			break;
